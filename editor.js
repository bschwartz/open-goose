$(document).ready(function(){
  $(".save.action").click(function(e){
    e.preventDefault();
    goose.writeFile(goose.currentPath(), $("#text-input").val(), function(file){
      $("#content").html(emojify(markdown.toHTML($("#text-input").val())));
      $("#markdown").hide();
      $("#preview").hide();
      $("#content").show();
      $("#text-input").val("");
  ...
  $('.edit.action').click(function(e) {
    e.preventDefault();
    goose.readFile(goose.currentPath(), function(data) {
      $("#markdown").show();
      $("#preview").show();
      $("#content").hide();
      $("#text-input").val(data);
  ...
  window.emojify = function(input) {
    var colontxt = new RegExp('\:[^\\s:]+\:', 'g');
    return input.replace(colontxt, function(m){
      var emojiid = m.substr(1, m.length-2);
      return "<img src='/emoji/"+emojiid+".png'/>";
    })
  }

  var editor = new Editor($("#text-input")[0], $("#preview")[0]);
  ...
  'goberserk', 'godmode', 'golf', 'goose', 'grapes', 'green_apple', 'green_book',
  ...
  { // emoji strategy
    ...
    replace: function (value) {
      console.log('blah');
      setTimeout(function(){editor.update();},50);
      return ':' + value + ': ';
    },
    index: 1,
    maxCount: 5
